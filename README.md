...
- **OpenAI API key** (for AI Blog Agent features)
...
# 2. Create .env (or copy from .env.example and fill in secrets)
#    Required: AI_LAB_LLM_OPENAI_API_KEY=sk-proj-...

# 3. Start Postgres + Redis
#    Ports: PostgreSQL :15432, Redis :16379
docker compose up -d

# 4. Backend — create venv & install deps
python -m venv .venv
...
# 6. Backend — start API server (port 18000)
python -m uvicorn backend.app.main:app --host 127.0.0.1 --port 18000

# 7. Backend — start Celery worker (in another terminal)
python -m celery -A backend.app.celery_app worker --loglevel=info -P solo

# 8. Frontend — install deps & build
cd frontend
npm install
npm run build

# 9. Frontend — start production server (port 13000)
npx next start --hostname 127.0.0.1 --port 13000

# 10. Open browser
#     http://127.0.0.1:13000/admin/login
#     Email:    admin@example.com
#     Password: test-admin-password-123456
```

> **Note**: Use `next start` (production) instead of `next dev` on Windows —
> Turbopack dev server has known socket-hang issues on Windows. The production
> build is stable.

### AI Blog Agent Pipeline

After login, navigate to **Ideas** in the admin sidebar:

1. Create a new idea manually (title, angle, target reader, goal)
2. **Approve** the idea → triggers outline generation (async via Celery + LLM)
3. Wait a few seconds, then **Approve** the outline → triggers draft writing
4. Wait ~10s, then **Approve** the draft → triggers technical review
5. View the technical review results (risk level, issues, suggestions)

Each async step runs via Celery and calls OpenAI's GPT-4o under the hood.

---

## Current State

This repository is in Harness v0 with an active AI Lab Portal application.
