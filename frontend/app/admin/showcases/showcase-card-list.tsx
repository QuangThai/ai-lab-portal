...
import type { AdminShowcase } from "./page";
...
export function ShowcaseCardList({ items, publishAction, unpublishAction }: Props) {
  if (items.length === 0) {
    return (
      <AdminEmptyState
        ctaHref="/admin/showcases/editor"
        ctaLabel="New showcase"
        description="Create your first showcase draft to publish client-ready work."
      ...
      {items.map((item) => {
        ...
        return (
          <AdminContentRow
            ...
            actions={
              <AdminListActions>
                <AdminListActionLink href={`/admin/showcases/${item.id}/edit`}>
                ...
                >
                  <input name="showcaseId" type="hidden" value={item.id} />
                  <AdminListSubmitButton
