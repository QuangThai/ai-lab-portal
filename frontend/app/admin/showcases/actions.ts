...
export async function publishFromListAction(formData: FormData) {
  ...
  if (typeof showcaseId !== "string" || showcaseId.length === 0) throw new Error("Missing showcaseId");
  await callMutation(`/admin/showcases/${showcaseId}/publish`, session);
}
...
export async function unpublishFromListAction(formData: FormData) {
  ...
  const showcaseId = formData.get("showcaseId");
  if (typeof showcaseId !== "string" || showcaseId.length === 0) throw new Error("Missing showcaseId");
  await callMutation(`/admin/showcases/${showcaseId}/unpublish`, session);
}
