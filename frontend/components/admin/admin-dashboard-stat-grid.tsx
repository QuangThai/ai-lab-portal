...
import { ArrowUpRight, FileText, Lightbulb, PencilLine } from "lucide-react";
...
const statAccents: Record<string, { bg: string; iconBg: string; ring: string }> = {
  "Blog posts": {
...
export function AdminDashboardStatGrid({ stats }: Props) {
  return (
    <section aria-labelledby="stats-heading" className="grid gap-5 lg:grid-cols-[11rem_minmax(0,1fr)]">
      {/* Side label */}
        ...
        {stats.map((stat) => {
          ...
          return (
            ...
            >
              ...
              {/* Number + label */}
              <div className="mt-4">
                <p className="text-xs font-medium text-muted-foreground/80">{stat.label}</p>
