...
### Recurring friction (deduplication)

When the same environment or harness pain appears on multiple tasks:

1. Record it once with `scripts/bin/harness-cli backlog add` (or a decision).
2. On later traces in the same period, reference the backlog id in `notes` or
   use `harness_friction: none` if nothing new occurred.
3. Do not copy the same boilerplate sentence (for example "workspace is not a
   git repository") into every trace — that hides product-level friction in
